...
# (optional: set the working directory to save outputs)
# setwd("C:/jiri/Dropbox/PHD/crowdsourcing/data/modis")

# parameters: selected.date
selected.date <- "2015-02-11"
...
legend("topright", legend = c("presence", "absence"), fill=c("lightblue", "green"))
plotCountries()
...
if (seltracks > 0) {
  ...
  if (nrow(garmin.tracks) > 0) {
    ...
    lines(garmin.tracks.utm, col="red")
  }
}
...
legend("topright", legend = c("p > 0.5", "p < 0.5"), fill=c("lightblue", "green"))
plotCountries()
